...
module Artoo
  module Drivers
    ...
    class Ps3Controller < Driver
      FIELDS = %w(l1 l2 r1 r2 square cross circle triangle
      select start left_joystick_press right_joystick_press ps
      hat_switch_left_right hat_switch_up_down left_joystick right_joystick)
      ...
      attr_accessor     :left_joystick

      attr_accessor     :right_joystick
      ...
      def consume_data(byte_array)
        ...
        left_joystick[:x] = joystick_coord_conf(byte_array[6]);
        left_joystick[:y] = -joystick_coord_conf(byte_array[7]);
        right_joystick[:x] = joystick_coord_conf(byte_array[8]);
        right_joystick[:y] = -joystick_coord_conf(byte_array[9]);
      end
      ...
      def emit_events
        FIELDS.each do |field|
          ...
          if Array === current_value
            publish(event_topic_name(field), *self.send(field))
          else
            publish(event_topic_name(field), self.send(field))
          end
        end
